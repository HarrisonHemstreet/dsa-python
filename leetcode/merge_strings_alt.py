...
class MergeAlternately():
    def run(self, word1: str, word2: str) -> str:
        longest: int = max(len(word1), len(word2))
        res: str = ""
        for i in range(longest):
            if self.safe_access(word1, i):
                res += word1[i]
            if self.safe_access(word2, i):
                res += word2[i]
        return res

    def safe_access(self, lst, index) -> bool | None:
        try:
            return lst[index]
        except IndexError:
            return None
