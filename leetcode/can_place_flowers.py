...
class CanPlaceFlowers():
    def run(self, flowerbed: list[int], n: int) -> bool:
        total: int = 0
        len_flowerbed: int = len(flowerbed) - 1
        for i, num in enumerate(flowerbed):
            if num == 0:
                if len_flowerbed == 0:
                    total += 1
                    flowerbed[i] = 1
                elif i == 0 and flowerbed[1] == 0:
                    total += 1
                    flowerbed[0] = 1
                elif i < len_flowerbed and flowerbed[i - 1] == 0 and flowerbed[i + 1] == 0:
                    total += 1
                    flowerbed[i] = 1
                elif i == len_flowerbed and flowerbed[i - 1] == 0:
                    total += 1
                    flowerbed[i] = 1
            if total >= n:
                return True
